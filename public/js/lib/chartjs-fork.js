		...
		draw: function() {
			...
			if (me._children.length > 0 && vm.fill) {
				...
				if (me._loop) {
					...
					var first = 0, last = me._children.length-1;

					while(first < last && me._children[first]._view.skip) first++;
					while(last > first && me._children[last]._view.skip) last--;

					//Round off the line by going to the base of the chart, back to the start, then fill.
					ctx.lineTo(me._children[last]._view.x, vm.scaleZero);
					ctx.lineTo(me._children[first]._view.x, vm.scaleZero);
				}
